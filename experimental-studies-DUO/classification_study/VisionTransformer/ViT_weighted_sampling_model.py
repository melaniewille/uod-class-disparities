## VisionTransformer Classification Model for DUO with Weighted Sampling
...
BASE_PATH = os.path.join(project_root, "classification_study", "cls_datasets", "imbalanced_data")
CHECKPOINTS_PATH = os.path.join(project_root, "classification_study", "VisionTransformer", "checkpoints", "checkpoints_weighted")
os.makedirs(CHECKPOINTS_PATH, exist_ok=True)
RESULTS_PATH = os.path.join(project_root, "classification_study", "cls_results", "VisionTransformer", "weighted")
...
if __name__ == '__main__': 
    ...
    for data_dir in dataset_variations:  # Loop over datasets
        ...
        # Initialize W&B
        wandb.init(
            project=f"classification_weighted_sampling",                  
            name=f"{run_name}",                         
            config={                                    
                "optimizer": "Adam",
        ...
        class_sample_count = torch.tensor([(np.array(targets) == t).sum() for t in np.unique(targets)])
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=weighted_sampler, num_workers=0)
